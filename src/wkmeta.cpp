...
Timestamp Timestamp::from_iso8601(std::string_view s) {
    ...
    if (dot_pos != std::string_view::npos) {
        ...
        if (end_pos == std::string_view::npos) end_pos = s.size();
...
static std::expected<Value, MetaError> parse_value(
    ...
    switch (type) {
        case TypeId::UINT8: {
            auto v = r.read_u8();
            if (!v) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            return Value{*v};
        }
        case TypeId::UINT16: {
            auto v = r.read_u16();
        ...
        case TypeId::RATIONAL: {
            auto n = r.read_i32();
            auto d = r.read_i32();
            if (!n || !d) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            return Value{Rational{*n, *d}};
        }
        case TypeId::LSTR: {
            auto lang_len = r.read_u8();
            if (!lang_len) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            auto lang = r.read_str(*lang_len);
            if (!lang) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            uint32_t text_len = value_size - 1 - *lang_len;
            auto text = r.read_str(text_len);
            if (!text) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            return Value{LocalizedString{std::move(*lang), std::move(*text)}};
        }
        case TypeId::STR: {
            auto s = r.read_str(value_size);
            if (!s) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            return Value{std::move(*s)};
        }
        case TypeId::BYTES: {
            auto b = r.read_bytes(value_size);
            if (!b) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            return Value{std::vector<uint8_t>(b->begin(), b->end())};
        }
        case TypeId::ARRAY: {
            auto count = r.read_u16();
            if (!count) return std::unexpected(MetaError{MetaErrorCode::ParseError});
            auto elem_type = r.read_u8();
            if (!elem_type) return std::unexpected(MetaError{MetaErrorCode::ParseError});
        ...
        case TypeId::STRUCT: {
            ...
            if (!field_count) return std::unexpected(MetaError{MetaErrorCode::ParseError});
...
auto MetaBlock::get(Namespace ns, uint16_t tag) const
    ...
    if (!e) return std::unexpected(MetaError{MetaErrorCode::TagNotFound});
    return &e->value;
}
...
auto MetaBlock::remove(Namespace ns, uint16_t tag) -> bool {
    auto it = std::remove_if(entries.begin(), entries.end(),
        [ns, tag](const Entry& e) { return e.ns == ns && e.tag == tag; });
    if (it != entries.end()) {
...
auto MetaBlock::get_geo_lat() const -> std::expected<double, MetaError> {
    ...
    if (auto* v = std::get_if<double>(*r)) return *v;
...
auto MetaBlock::get_capture_ts() const -> std::expected<Timestamp, MetaError> {
    ...
    if (!r) return std::unexpected(r.error());
    if (auto* v = std::get_if<Timestamp>(*r)) return *v;
...
auto MetaBlock::get_license() const -> std::expected<std::string, MetaError> {
    ...
    if (!r) return std::unexpected(r.error());
    if (auto* v = std::get_if<std::string>(*r)) return *v;
...
auto MetaBlock::parse(std::span<const uint8_t> data)
    ...
    auto ver = r.read_u8();
    if (!ver) return std::unexpected(MetaError{MetaErrorCode::ParseError, "truncated"});
    if (*ver != 0x01) {
        return std::unexpected(MetaError{MetaErrorCode::ParseError,
...
static std::string namespace_name(Namespace ns) {
    switch (ns) {
        case Namespace::Capture:  return "capture";
        case Namespace::Geo:      return "geo";
        case Namespace::Time:     return "time";
        case Namespace::Rights:   return "rights";
        case Namespace::Content:  return "content";
        case Namespace::Anim:     return "anim";
        case Namespace::Region:   return "region";
        case Namespace::Device:   return "device";
        case Namespace::Rating:   return "rating";
        case Namespace::Custom:   return "custom";
        case Namespace::ProvRef:  return "provRef";
        default: return "unknown_" + std::to_string(static_cast<int>(ns));
    }
}
