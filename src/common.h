...
public:
    ...
    [[nodiscard]] Result<uint8_t> read_u8() {
        if (remaining() < 1) return std::unexpected(Error{ErrorCode::TruncatedInput, "need 1 byte"});
        return data_[pos_++];
    }
    [[nodiscard]] Result<uint16_t> read_u16() {
        if (remaining() < 2) return std::unexpected(Error{ErrorCode::TruncatedInput, "need 2 bytes"});
        auto v = read_le16(data_.data() + pos_);
        pos_ += 2;
        return v;
    }
    [[nodiscard]] Result<uint32_t> read_u32() {
        if (remaining() < 4) return std::unexpected(Error{ErrorCode::TruncatedInput, "need 4 bytes"});
        auto v = read_le32(data_.data() + pos_);
        pos_ += 4;
        return v;
    }
    [[nodiscard]] Result<uint64_t> read_u64() {
        if (remaining() < 8) return std::unexpected(Error{ErrorCode::TruncatedInput, "need 8 bytes"});
    ...
    [[nodiscard]] Result<std::span<const uint8_t>> read_bytes(size_t n) {
        if (remaining() < n) return std::unexpected(Error{ErrorCode::TruncatedInput});
...
inline uint8_t channels_for_format(PixelFormat fmt) {
    switch (fmt) {
        ...
        case PixelFormat::Y16:    return 1;
        case PixelFormat::YA8:    return 2;
        case PixelFormat::YA16:   return 2;
    }
    return 0;
}
